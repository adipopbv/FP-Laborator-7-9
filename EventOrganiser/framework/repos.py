...
class Repo:
    ...
    def selection_sort(self, list, key, reversed, cmp):
        for i in range(0, len(list)-1):
            ind = i
            # find the smallest element in the rest of the list
            for j in range(i+1, len(list)):
                if cmp(key(list[j]), key(list[ind])) == -1:
                    ind = j
            if reversed:
                if i < ind:
                    # interchange
        ...
        if reversed:
            ...
            for i in range(len(list)):
                rev_list.append(list[-1*(i+1)])
            list = rev_list
            return rev_list
        return list

    def shake_sort(self, list, key, reversed, cmp):
        left = 0
        right = len(list) - 1
        last_swap = 0
        while left < right:
